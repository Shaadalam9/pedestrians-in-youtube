...
def download_video_with_resolution(video_ids, resolutions=["2160p", "1440p", "1080p", "720p", "480p", "360p"],
                                   source_videos="."):
    video_ids = [id.strip() for id in video_ids.strip("[]").split(',')]
    for vid in video_ids:
        # Check if the MP4 file already exists in the output folder
        mp4_file_path = os.path.join(source_videos, f"{vid}.mp4")
        ...
        # Download
        try:
            selected_stream.download(source_videos, filename=f"{vid}.mp4") 
        except Exception as e:
            logger.error("Error occurred {}", e)
    return len(video_ids)  # return number of videos processed
